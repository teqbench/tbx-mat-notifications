...
Uses `actions/create-github-app-token@v3` to create a short-lived token from the `teqbench-automation` GitHub App. Dependency-update PRs run as this app (via [Renovate ↗](https://docs.renovatebot.com/) — see the [org-level Renovate doc ↗](https://github.com/teqbench/.github/blob/main/renovate.md)), so the app token is available on every CI run, including automated dependency updates.
